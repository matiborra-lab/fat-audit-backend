  ...
  app.get('/api/dashboard', async (req, res) => {
    try {
      ...
      const { rows: conRn } = await db.query(
        `SELECT * FROM (
           SELECT id, sucursal_id, puntaje_total, semaforo, resultado, completada_en, detalle_calculo,
      ...
      // Promedio por sector/area de las ultimas 10 auditorias completadas
      // en el alcance actual (o de la unica sucursal si es una sola).
      const { rows: recientes } = await db.query(
        `SELECT detalle_calculo FROM audit_runs WHERE sucursal_id = ANY($1) AND estado = 'COMPLETADA'
         ORDER BY completada_en DESC LIMIT 10`,
        [sucursalIds]
      );
      ...
      res.json({ sucursales, ranking, promedioPorArea });
    } catch (err) {
      res.status(500).json({ error: err.message });
    }
  });
