...
// req.usuario = { usuarioId, rol, sucursal_id, activo, email, nombre, personal_marca, tutorial_completado_en }.
...
async function requireAuth(req, res, next) {
  ...
  try {
    ...
    const { rows } = await db.query(
      "SELECT rol, sucursal_id, activo, email, TRIM(nombre || ' ' || COALESCE(apellido, '')) AS nombre, personal_marca, tutorial_completado_en FROM usuarios WHERE id = $1",
      [payload.usuarioId]
    );
    const fila = rows[0];
    ...
    req.usuario = {
      ...
      email: fila.email,
      nombre: fila.nombre,
      personal_marca: fila.personal_marca === true,
